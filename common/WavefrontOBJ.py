...
def load_obj( filename: str, default_mtl='default_mtl', triangulate=False) -> WavefrontOBJ:
    ...
    def parse_vertex( vstr ):
        vals = vstr.split('/')
        vid = int(vals[0])-1
        tid = int(vals[1])-1 if len(vals) > 1 and vals[1] != 'X' and vals[1] else -1
        nid = int(vals[2])-1 if len(vals) > 2 and vals[2] != 'X' else -1
        return (vid,tid,nid)

    with open( filename, mode='r' ) as objf:
        obj = WavefrontOBJ(default_mtl=default_mtl)
        obj.path = filename
        cur_mat = obj.mtls.index(default_mtl)
        vappend = obj.vertices.append
        nappend = obj.normals.append
        tappend = obj.texcoords.append
        pappend = obj.polygons.append
        mtappend = obj.mtlid.append
        ...
        for line in objf:
            ...
            if toks[0] == 'v':
                ...
                vappend([float(toks[1])])
                vappend([float(toks[2])])
                vappend([float(toks[3])])
                #vappend([float(1.0)])
            elif toks[0] == 'vn':
                # obj.normals.append( [ float(v) for v in toks[1:]] )
                nappend([float(toks[1])])
                nappend([float(toks[2])])
                nappend([float(toks[3])])
            elif toks[0] == 'vt':
                # obj.texcoords.append( [ float(v) for v in toks[1:]] )
                tappend([float(toks[1])])
                tappend([float(toks[2])])
                tappend([float(toks[3])])
            elif toks[0] == 'f':
                # poly = [ parse_vertex(vstr) for vstr in toks[1:] ]
                poly = [parse_vertex(toks[1]), parse_vertex(toks[2]), parse_vertex(toks[3])]
                if triangulate:
                    for i in range(2,len(poly)):
                        mtappend( cur_mat )
                        pappend( (poly[0], poly[i-1], poly[i] ) )
                else:
                    mtappend(cur_mat)
                    pappend( poly )
            elif toks[0] == 'mtllib':
    ...
    with open( filename, 'w' ) as ofile:
        ...
        for pid in poly_idx:
            ...
            for v in obj.polygons[pid]:
                vstr = '{}/{}/{} '.format(v.item()+1,v.item()+1 if v.item() >= 0 else 'X','X' )
                pstr += vstr
            ofile.write( pstr+'\n')
